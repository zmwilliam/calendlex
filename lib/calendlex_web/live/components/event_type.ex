defmodule CalendlexWeb.Components.EventType do
  ...
  def day(%{index: index, current_path: current_path, date: date, time_zone: time_zone} = assigns) do
    ...
    class =
      class_list([
        ...
        {"text-blue-200 cursor-default pointer-events-none", disabled}
      ])
  ...
  def time_slot(
    ...
    ~H"""
    <%= live_redirect to: @schedule_path, 
        class: "text-center block w-full p-4 mb-2 font-bold text-blue-600 border border-blue-300 rounded-md hover:border-blue-600" do %>
      <%= @text %>
    <% end %>
    """
  end
